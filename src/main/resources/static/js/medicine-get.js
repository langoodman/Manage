window.onload = function () {
    var url = '/medicine/getMedicineData?';
    var suburl = "/medicine/operationMedicine";
    var content = document.getElementById('mainbody');
    var tmp = '';
    var list = 0;
    ajax({
        url: '/medicine/getMedicineDataCount', //和下面的url不一样
        type: "get",
        dataType: "json",
        ...
        ajax({
            ...
            success: function (data) {
                ...
                list.forEach(function (context) {
                    tmp += '<tr style="background: rgb(255, 255, 255) none repeat scroll 0% 0%;"><td><input type="checkbox" value="" editable="false" name="checkbox"></td><td name="id" style="display:none;">' + context.id + '</td><td name="name">' + context.name + '</td><td name="number">' + context.number + '</td><td name="origin">' + context.origin + '</td><td name="specification">' + context.specification + '</td><td name="price">' + context.price + '</td><td name="stock">' + context.stock + '</td><td><button type="button" class="btn btn-warning btnSubmit" onclick="submitmessage(this,\'' + suburl + '\')">提交 </button></td><tr>';
